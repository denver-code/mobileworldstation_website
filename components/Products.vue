<template>
  <div :class="[detail ? 'detail' : '']" class="shadow-xl shadow-slate-300/60">
    ...
    <div class="text-wrapper p-4">
      ...
      <div class="content is-clearfix">
        ...
        <div class="flex justify-between mt-5 items-center">
          <select class="p-2 border-2 " @change="onSelectQuantity(product.id)" v-model="selected">
            <option
              v-bind:key="quantity"
          ...
          <button class=" p-3 bg-blue text-white" v-if="!product.isAddedToCart" @click="addToCart(product.id)">{{ addToCartLabel }}</button>
          <button class=" p-3" v-if="product.isAddedToCart" @click="removeFromCart(product.id, false)">{{ removeFromCartLabel }}</button>
        </div>
      </div>
    </div>
  </div>
